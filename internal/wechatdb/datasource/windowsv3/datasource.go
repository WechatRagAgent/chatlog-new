...
func (ds *DataSource) GetMessagesCount(ctx context.Context, startTime, endTime time.Time, talker string) (int64, error) {
	...
	for _, talkerItem := range talkers {
		...
		// 构建表名（windowsv3使用Chat_前缀）
		tableName := "Chat_" + hex.EncodeToString([]byte(talkerItem))
		...
		log.Debug().Msgf("WindowsV3 Count query for table: %s", tableName)
		...
		if err != nil {
			...
			log.Err(err).Msgf("从WindowsV3数据库查询消息数量失败")
			continue
		}

		totalCount += count
		log.Debug().Msgf("WindowsV3 Table %s count: %d, total: %d", tableName, count, totalCount)
	}
